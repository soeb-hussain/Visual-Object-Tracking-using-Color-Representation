...
count = 0
max_obj=0
correct = fbBoundingBox[:]
while run:
    print(fbBoundingBox[:2])
    ...
    obj = s_F.sum()*s_D#(1 - s_B.sum())#*s_D#*s_D#

    print(obj,max_obj)
    if max_obj<=obj and count==1:
        max_obj=obj
        correct = fbBoundingBox[:]
    print('.................',correct[1],correct[0],max_obj)
    ...
    for i in range(len(x_i)):
        t = np.asarray([  x_i[i] , y_i[i]  ])
        #print(sum_RGB,s_ref[sum_RGB[i]]
        su = su + t*s_ref[sum_RGB[i]]/s_ref.sum()
    su[0] = su[0]/max(x_i)
    su[1]=su[1]/max(y_i)
    print(su)
    ...
    if count ==0:
        ...
        fbBoundingBox[0]= round(fbBoundingBox[0]-50)
        fbBoundingBox[1]=round(fbBoundingBox[1]+30)
        

    else:
        fbBoundingBox[0]= fbBoundingBox[0]+ su[1]
        fbBoundingBox[1]= fbBoundingBox[1] + su[0] #(ratioMIN)/math.sqrt(min(u))
    temp = 1
