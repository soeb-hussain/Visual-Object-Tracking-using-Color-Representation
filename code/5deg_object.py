...
while run:
    ...
    # #if bhatta_dist(hist, hist_ref_in)<0.001:
    #     run = False
    #     print('cvcv')
    ...
    # if abs(s_ref.sum() - s_new.sum())<0.1:
    #     run = False
    EP =1
    ...
    for i in range(len(x_i)):
        ...
        su_trans = su_trans + t*s_ref[sum_RGB[i]]/s_ref.sum()
    ...
    if count ==0:
        ...
        fbBoundingBox[2]= round(fbBoundingBox[2]*.56)
        fbBoundingBox[3]=round(fbBoundingBox[3]*1.2)
        ...
        fbBoundingBox[1]=fbBoundingBox[1]+su[0]
