...
def getfbBBoxMask(fbBoundingBox,img,temp):
    ...
    if temp!=3:
        if temp==0:
            cv2.drawContours(original_o,[ctr[0]],0,(0,0,1),1)
        else:
...
rotated90 = deepcopy(original)
trash,x_i_trashi,y_itrashi,sum_rgb_trash,hist_ref_out,trash,trash = getfbBBoxMask(out_region,rotated90,3)
...
while run:
    ...
    rotated90 = deepcopy(original)
    raw_image,x_iy,y_iy,sum_RGBy,hist_out,Cx,Cy = getfbBBoxMask(fbBoundingBox_out,rotated90,3)
