...
cv2.waitKey(0)
region = [original.shape[0]/2 - 20 ,original.shape[1]/2 - 60,original.shape[0]/7,original.shape[1]/21]
...
ratio = math.sqrt(max(u))/max(region[3],region[2])
print (abs(min(u)/min(region[3],region[2])),'8-8-8-8-8',ratio)
...
temp=0
while run:
    fbBoundingBox[3]=(math.sqrt(min(u))/ratio + fbBoundingBox[3])//2
    fbBoundingBox[2]=(math.sqrt(max(u))/ratio + fbBoundingBox[2])//2
    print(fbBoundingBox[3],fbBoundingBox[2],region[3],region[2])
    rotated90 = deepcopy(original)
    ...
    out_region = region[:]
    for i in range(2,4):
        out_region[i] = round(fbBoundingBox[i]*1.414) 
    ...
    for i in range(len(x_i)):
        ...
        yo = np.matmul(p,t)
        #print(sum_RGB,s_ref[sum_RGB[i]])
        su = su + yo*s_ref[sum_RGB[i]]/s_new.sum()
