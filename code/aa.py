    ...
    for j in range(m):
        for i in range(l):
            if grid[i,j]==False:
                ...
                x_i.append(j-m//2)
                y_i.append(i-l//2)
                t = ((raw_image[j][i][0]//32) * 64) + ((raw_image[j][i][1]//32) * 8) + (raw_image[j][i][2]//32)
...
cv2.waitKey(0)
region = [original.shape[0]/2 ,original.shape[1]/2,original.shape[0]/10,original.shape[1]/10]
...
hist_ref_in,hist_ref_out,Sum_rgb = tracker_dat_intialize(original,region)
...
#otated90 = cv2.warpAffine(original, M, (h, w))

rotated90 = np.rot90(original)
#rotated90 = deepcopy(original)

# cv2.imshow('Image rotated by 90 degrees',rotated90)
# cv2.waitKey(0) # waits until a key is pressed
# anAngle = 0
region = [region[1],region[0],region[2]-10,region[3]-10]
# raw_image,x_i,y_i,sum_RGB,hist_ref_in,Cx,Cy = getfbBBoxMask(region,original)
# out_region = region[:]
# for i in range(2,4):
#     out_region[i] = round(region[i]*1.414) 

# raw_image,x_iy,y_iy,sum_RGBy,hist_out,Cx,Cy = getfbBBoxMask(out_region,original)

# s = hist/(hist_out + 0.000000001)

#Cropped_Img = rotated90[(region[0]-region[2]):(region[0]+region[2]),(region[1]-region[3]):(region[1]+region[3])]
#print('asdasddasdsadasda')
#hist_ref_in,hist_ref_out,Sum_rgb = tracker_dat_intialize(rotated90,region)
#new_s = hist_ref_in/(hist_ref_out+0.000000002)

#cv2.imshow('Cropped check1' ,Cropped_Img)
#cv2.waitKey(0)
#print()
angle = 0
correction = 0

rotated90_o = deepcopy(rotated90)

fbBoundingBox = region + [angle+correction]
while True:
    fbBoundingBox[4]=angle+correction
    print(fbBoundingBox[4])
    rotated90 = deepcopy(rotated90_o)
    raw_image,x_i,y_i,sum_RGB,hist,Cx,Cy = getfbBBoxMask(fbBoundingBox,rotated90)

    print('~~~~~~~~~',bhatta_dist(hist, hist_ref_in),'~~~~~~~~~')
    #cv2.imshow('asd',raw_image)
    #cv2.waitKey(0)
    out_region = region[:]
    for i in range(2,4):
    ...
    raw_image,x_iy,y_iy,sum_RGBy,hist_out,Cx,Cy = getfbBBoxMask(fbBoundingBox,rotated90)

    s_new = hist/(hist_out + 0.000000001)

    print('------',s_ref.sum(),s_new.sum(),'------')
    ...
    # for i in range(l):
    #     for j in range(m):
    #         t = [[round(i-l/2),round(j-m/2)]]
    #         x = [[round(i-l/2)],[round(j-m/2)]]
    #         yo = np.matmul(x,t)
    #         su = su + yo*s_ref[Sum_rgb[i][j]]/s_ref.sum()
    #         #print(su)
    #print(len(x_i),len(hist),len(sum_RGB),len(sum_RGBy))
    ...
    #print((180*arccos(clip(c, -1, 1))/3.14))# -> cosine of the angle
    #print('***********************',correction,angle,c)
    #correction = correction + 10*(180*arccos(clip(c, -1, 1))/3.14) #+ 1 
    # if correction==fbBoundingBox[4]:
    #     if temp==0:
    #         temp=1
    #     else:
    #         temp=0
